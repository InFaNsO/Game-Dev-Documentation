...
- **Locked design — Mega Structure interiors (Topic 8, with corrections).** Every structure visit follows a shared **8-beat shape**: Approach → Entry Hub → Exploration → Puzzle Gate → Inner Sanctum → Reactivation (multi-phase: locate → restore → defend → activate, boss is the defense phase) → Blueprint pickup → Outro. **Per-structure signature mechanics**: Lithic Mow = vertical drill descent + reactivation (Bhu-Mani gear); Genesis Vats = vapor management (toxic vs healing vapor flows, valve redirection); Prism Forge = light/laser redirection + heat zones (Agni-Mani gear extends safe exposure). **Lore distribution hybrid**: audio logs primary + purified-NPC dialogue for emotional landing. **Light environmental puzzles** (3-5 per structure, using the signature mechanic). **Authored sections**: hub, boss arena, puzzles, reactivation phases, inner sanctum. **Procgen sections**: regular raid/exploration zones. Scope target: 5-8 hours of main-path content per structure.
  ...
  **CORRECTIONS / IMPORTANT REVISIONS:**
  ...
  - **Named Husk distribution corrected**: **Lithic Mow = 0 named Husks** (Grinder-only biome). **Genesis Vats = 2 mandatory** (Husk 1 entry hub teaches Jal; Husk 2 inner sanctum teaches Vayu + reveals conspiracy). **Prism Forge = 1 mandatory + 1 optional** (Husk 1 entry hub teaches Agni; optional Husk 4 in inner sanctum side area = head of Accord research, provides Akash refinement knowledge + bonus weapons for Sun-King fight).
  - **Sun-King at Prism Forge = the campaign's final boss fight.**
  - **Akash Mani IS produced in this game** (revision to earlier Topic 4 framing). A NEW Akash-Mani is refined at endgame as the climactic act of the multi-species (good) ending — the original is still destroyed/historical-only, but the player produces a new one correctly using all four elemental Manis + multi-species cooperation as the stabilizing protocol the Accord lacked. Engaging the optional Forge Husk makes this refinement clean; skipping makes it tense and improvised but still successful.
  - **Forced purification (no kill option)**: named Husks CANNOT be killed. Player must defeat them in combat and purify them by force. The Husk teaches the Mani refinement as a dying-act knowledge transmission AT CRITICAL STATE during the fight — knowledge transfer + forced-purification QTE are unified into one beat. *"Take my knowledge, then take my pain."* All 3 mandatory Husks become allies at Council Scene for every player; optional Husk 4 is the only roster variance.
  - **Scope target revised**: ~3-4 hours of main-path content per structure → ~12-15 hours total main game (working estimate, finalized during dev).
