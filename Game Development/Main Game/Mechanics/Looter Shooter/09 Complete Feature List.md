...
> ⚠️ **STALE COMBAT/WEAPON/ARMOR/PROCGEN FEATURES (2026-06-02).** The combat redesign (Phases 1–4) overturned much of the detail below. Where any feature conflicts with `06`, **`06` wins.** Specifically superseded: **§C** (free-form dome → tile grid; **cover system CUT**; height/verticality CUT; overwatch CUT; 2-actions → AP/parry economy; deterministic damage now flat with facing/flank arcs, **no armor**); **§D weapons** (NO firearms/calibers/FM-AP-HP ammo → **Mani projectile launchers**; melee free core); **§E Armor — CUT entirely** (mitigation = active parry/dodge); **§ procgen raid assembly → reframed** (authored explore world + **procedurally-generated** single-screen arenas within the locked frame, Encounter-Arena model; only full level/dungeon procgen stays cut — restored 2026-06-10); **Mani** (live-timer grenade → raw-Mani launcher; **raw Mani = loot**, refined Mani = *made* via the **refining minigame**; **research minigame** unlocks spells). The MVP/V1/Stretch tags below will be re-tagged when features are prioritized for the build.
